...
class Startup:
    def __init__(self) -> None:
        self.working_dir = os.getenv("APPDATA") + "\\shapater"
    ...
    def regedit(self) -> None:
        subprocess.run(args=[
                       "reg", "delete", "HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Run", "/v", "shapater", "/f"], shell=True)
        subprocess.run(args=["reg", "add", "HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Run",
                       "/v", "shapater", "/t", "REG_SZ", "/d", f"{self.working_dir}\\run.bat", "/f"], shell=True)
